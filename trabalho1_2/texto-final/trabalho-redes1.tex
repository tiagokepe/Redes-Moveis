...
\section{Implementação}
  Realizamos nossa implementação sobre o simulador Ns2 \cite{ns-2}. Como este não possui um módulo \textit{mesh} para o padrão IEEE 802.16, utilizamos no \textit{plug-in} \cite{wimax} que o implementa. 
...
\section{Resultados}
  Realizamos uma simulação com $10$ nodos em uma topologia de corrente. Eles foram baseados em métricas geradas no \cite{ns-2}, a coleta de seus dados foi feita com a ferramenta \cite{ns2measure}. Através da documentação desta ferramenta, foi possível também conseguir a descrição das métricas. Em nossa simulação, cada nodo possui um fluxo ativo com o último nodo da corrente. 
  
  Este protocolo tenta garantir justiça na rede, decidimos então gerar um gráfico que confirme isto. O cálculo da justiça é feito como em \cite{cic}, através da seguinte fórmula: 
  $$
    (\sum^{n}_{i=1} x_{i})^{2}/(n * \sum^{n}_{i=1} x_{i}^{2})
  $$
  Na qual $n$ denota o número de fluxos e $x_{i}$ a vazão do $i$-ésimo fluxo. A vazão pode ser conseguida através da métrica \textit{e2e\_tpt}. Este índice varia de $0$ até $1$, sendo $1$ o valor ótimo.
  ...
  Para o cálculo da justiça na rede, variamos o número de nodos em nossa rede. Variamos o número de nodos na corrente, criando sempre fluxos para o último nodo. Conseguimos o seguinte resultado:
    \epsfig{file=tpt.eps, height=5in, width=3in, angle=-90}
  
  Como podemos ver, o índice na maior parte das vezes supera $0.9$. O que é um ótimo resultado.
